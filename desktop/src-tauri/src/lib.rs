...
const PEER_PIECE_BYTES: usize = 4 * 1024 * 1024;
...
struct SyncChange {
    ...
    #[serde(default)]
    peer_pieces: Vec<PeerPieceSource>,
    #[serde(default)]
...
#[derive(Debug, Deserialize, Clone)]
struct PeerPieceSource {
    device_id: String,
    endpoint_url: String,
    #[serde(rename = "piece_index")]
    piece_index: i64,
    #[serde(rename = "piece_size_bytes")]
    piece_size_bytes: i64,
...
#[derive(Debug, Serialize, Clone)]
struct PeerPieceManifest {
    index: i64,
    #[serde(rename = "pieceSizeBytes")]
    piece_size_bytes: i64,
...
fn app_data_dir() -> Result<PathBuf> {
    Ok(local_app_base_dir())
}

fn logs_dir() -> PathBuf {
    local_app_base_dir().join("logs")
}
...
fn install_panic_logger() {
    std::panic::set_hook(Box::new(|panic_info| {
        ...
        append_desktop_log("panic", &full_message);
        show_startup_error(
            "Torrent Shred Vault startup failed",
            &format!(
                "The desktop app hit a startup error and wrote a log file.\n\n{full_message}\n\nLogs: {}",
...
fn get_bandwidth_limit_kbps(conn: &Connection) -> Result<i64> {
    Ok(get_setting(conn, "bandwidth_limit_kbps")?
        .and_then(|value| value.parse::<i64>().ok())
        .filter(|value| *value >= 0)
        .unwrap_or(0))
}

fn set_bandwidth_limit_kbps(conn: &Connection, value: i64) -> Result<()> {
    set_setting(conn, "bandwidth_limit_kbps", &value.clamp(0, 1_000_000).to_string())
}
...
pub(crate) fn log_activity(state: &AppState, level: &str, message: &str) -> Result<()> {
    append_desktop_log(level, message);
    let conn = state.connect()?;
    conn.execute(
...
fn build_piece_manifest(path: &Path) -> Result<Vec<PeerPieceManifest>> {
    let mut file = fs::File::open(path).with_context(|| format!("failed to open {}", path.display()))?;
    let mut pieces = Vec::new();
    let mut offset = 0_i64;
    let mut index = 0_i64;
    let mut buffer = vec![0_u8; PEER_PIECE_BYTES];
    loop {
        let read = file.read(&mut buffer).with_context(|| format!("failed to read {}", path.display()))?;
        if read == 0 {
            break;
        }
        pieces.push(PeerPieceManifest {
            index,
            piece_size_bytes: PEER_PIECE_BYTES as i64,
            offset_bytes: offset,
            size_bytes: read as i64,
            sha256: hash_bytes(&buffer[..read]),
        });
        offset += read as i64;
        index += 1;
    }
...
fn publish_local_catalog(state: &AppState, config: &RuntimeConfig) -> Result<()> {
    ...
    for row in rows {
        ...
        let pieces = build_piece_manifest(&full_path).unwrap_or_default();
        let _ = publish_peer_availability(config, &root_kind, &relative_path, revision, size_bytes, &sha256, &pieces);
    }
    Ok(())
}
...
fn reconstruct_file_from_raid(
    ...
    if let Some(hash) = change.sha256.as_deref() {
        ...
        let _ = publish_peer_availability(config, root_kind, &change.path, change.revision, raid.size_bytes, hash, &pieces);
    }
    Ok(true)
}
...
impl ProgressReader {
    fn new(file: fs::File, state: AppState, root_kind: &str, relative_path: &str, direction: &str, bytes_total: i64, bandwidth_limit_kbps: i64) -> Self {
        Self {
            file,
            state,
...
impl Read for ProgressReader {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        if read > 0 {
            ...
            throttle_transfer(self.bytes_done, self.started_at, self.bandwidth_limit_kbps);
...
fn throttle_transfer(bytes_done: i64, started_at: Instant, bandwidth_limit_kbps: i64) {
    if bandwidth_limit_kbps <= 0 || bytes_done <= 0 {
        return;
    }
    let bytes_per_second = bandwidth_limit_kbps.saturating_mul(1024).max(1) as u128;
    let expected_millis = (bytes_done.max(0) as u128).saturating_mul(1000) / bytes_per_second;
    let elapsed_millis = started_at.elapsed().as_millis();
    if expected_millis > elapsed_millis {
        let sleep_for = (expected_millis - elapsed_millis).min(5_000) as u64;
...
fn stream_response_to_file(
    ...
    let bandwidth_limit_kbps = state
        .connect()
        .ok()
        .and_then(|conn| get_bandwidth_limit_kbps(&conn).ok())
        .unwrap_or(0);
    ...
    loop {
        ...
        throttle_transfer(transferred, started_at, bandwidth_limit_kbps);
...
fn piece_part_dir(target_path: &Path) -> PathBuf {
    let name = target_path
        .file_name()
        .and_then(|value| value.to_str())
        .unwrap_or("download");
    target_path.with_file_name(format!("{name}.tsv-parts"))
}

fn validate_piece_file(path: &Path, expected_hash: &str) -> bool {
    path.is_file()
        && hash_file(path)
            .map(|(actual, _)| actual.eq_ignore_ascii_case(expected_hash))
            .unwrap_or(false)
}

fn download_from_peer_pieces(
    state: &AppState,
    config: &RuntimeConfig,
    change: &SyncChange,
    root_kind: &str,
    ...
    if change.peer_pieces.is_empty() {
        return Ok(false);
    }
    let mut by_index: HashMap<i64, Vec<PeerPieceSource>> = HashMap::new();
    let mut expected_size = 0_i64;
    for piece in &change.peer_pieces {
        by_index.entry(piece.piece_index).or_default().push(piece.clone());
        expected_size = expected_size.max(piece.offset_bytes + piece.size_bytes);
    }
    let Some(max_index) = by_index.keys().max().copied() else {
        return Ok(false);
    };
    ...
    let parts_dir = piece_part_dir(target_path);
    fs::create_dir_all(&parts_dir)?;
    let client = auth_client()?;
    let total = expected_size.max(0);
    let mut bytes_done = 0_i64;
    let started_at = Instant::now();
    upsert_transfer_progress(state, root_kind, &change.path, "download", "pieces", "active", total, 0, Some("Downloading file pieces"))?;

    for index in 0..=max_index {
        let sources = by_index.get(&index).cloned().unwrap_or_default();
        let expected_hash = sources.first().map(|piece| piece.sha256.clone()).unwrap_or_default();
        let part_path = parts_dir.join(format!("{index}.part"));
        if validate_piece_file(&part_path, &expected_hash) {
            bytes_done += sources.first().map(|piece| piece.size_bytes).unwrap_or(0);
            throttle_transfer(bytes_done, started_at, config.bandwidth_limit_kbps);
            upsert_transfer_progress(state, root_kind, &change.path, "download", "pieces", "active", total, bytes_done, Some("Reusing downloaded piece"))?;
            continue;
        }
        ...
        for source in sources {
            let request_url = reqwest::Url::parse_with_params(
                &format!("{}/peer/piece", source.endpoint_url.trim_end_matches('/')),
                &[("ticket", source.ticket.as_str())],
            )
            ...
            match response {
                Ok(response) if response.status().is_success() => {
                    let bytes = response.bytes().context("failed to read peer piece")?.to_vec();
                    if bytes.len() as i64 != source.size_bytes {
                        let _ = log_activity(
                            state,
                            "warn",
                            &format!("Peer {} returned wrong piece size for {}", source.device_id, change.path),
                        );
                        continue;
                    }
                    ...
                    if !actual_hash.eq_ignore_ascii_case(&source.sha256) {
                        let _ = log_activity(
                            state,
                            "warn",
                            &format!("Peer {} returned invalid piece hash for {}", source.device_id, change.path),
                        );
                        continue;
                    }
                    fs::write(&part_path, &bytes).with_context(|| format!("failed to write {}", part_path.display()))?;
                    bytes_done += source.size_bytes;
                    throttle_transfer(bytes_done, started_at, config.bandwidth_limit_kbps);
                    downloaded = true;
                    upsert_transfer_progress(state, root_kind, &change.path, "download", "pieces", "active", total, bytes_done, Some("Downloading file pieces"))?;
                    break;
                }
                Ok(response) => {
                    let body = response.text().unwrap_or_else(|_| "peer piece download failed".to_string());
                    let _ = log_activity(
                        state,
                        "warn",
                        &format!("Peer {} rejected piece {} for {}: {}", source.device_id, index, change.path, body),
                    );
                }
                Err(error) => {
                    let _ = log_activity(
                        state,
                        "warn",
                        &format!("Peer {} unavailable for piece {} of {}: {}", source.device_id, index, change.path, error),
                    );
                }
            }
        }
    ...
    let name = target_path.file_name().and_then(|value| value.to_str()).unwrap_or("download");
    let temp_path = target_path.with_file_name(format!("{name}.download"));
    {
        let mut output = fs::File::create(&temp_path).with_context(|| format!("failed to create {}", temp_path.display()))?;
        for index in 0..=max_index {
            let part_path = parts_dir.join(format!("{index}.part"));
            let mut input = fs::File::open(&part_path).with_context(|| format!("failed to open {}", part_path.display()))?;
            std::io::copy(&mut input, &mut output).with_context(|| format!("failed to assemble {}", target_path.display()))?;
        }
    }
    if let Some(expected_hash) = change.sha256.as_deref() {
        let (actual_hash, _) = hash_file(&temp_path)?;
        if !actual_hash.eq_ignore_ascii_case(expected_hash) {
            return Err(anyhow!("assembled peer file hash mismatch for {}", change.path));
        }
    }
    ...
    if let Some(hash) = change.sha256.as_deref() {
        let pieces = build_piece_manifest(target_path).unwrap_or_default();
        let _ = publish_peer_availability(config, root_kind, &change.path, change.revision, total, hash, &pieces);
    }
    upsert_transfer_progress(state, root_kind, &change.path, "download", "completed", "completed", total, total, Some("Piece download completed"))?;
    Ok(true)
}
        ...
        match response {
            Ok(response) if response.status().is_success() => {
                ...
                if let Some(hash) = change.sha256.as_deref().or(source.sha256.as_deref()) {
                    let pieces = build_piece_manifest(target_path).unwrap_or_default();
                    let _ = publish_peer_availability(config, root_kind, &change.path, change.revision, source.size_bytes.max(0), hash, &pieces);
                }
                return Ok(true);
            }
            Ok(response) => {
...
fn download_remote_file(
    ...
    if config.transport_mode == "peer" && download_from_peer_pieces(state, config, change, root_kind, target_path)? {
        return Ok(());
    }
...
fn handle_peer_client(state: AppState, mut stream: TcpStream) -> Result<()> {
    ...
    if request_url.path() != "/peer/file" && request_url.path() != "/peer/piece" && request_url.path() != "/peer/raid-shard" {
        let _ = write_http_response(
            &mut stream,
            "404 Not Found",
            &[("Content-Length", "0".to_string())],
    ...
    let payload = if request_url.path() == "/peer/file" {
    ...
    } else if request_url.path() == "/peer/piece" {
        if validated.ticket.kind != "peer_file_piece" {
            let _ = write_http_response(&mut stream, "404 Not Found", &[("Content-Length", "0".to_string())], None);
            return Ok(());
        }
        ...
        let offset = validated.ticket.offset_bytes.unwrap_or(0).max(0) as u64;
        let size = validated.ticket.size_bytes.unwrap_or(0).max(0) as usize;
        let mut file = fs::File::open(&file_path).with_context(|| format!("failed to open {}", file_path.display()))?;
        file.seek(SeekFrom::Start(offset)).with_context(|| format!("failed to seek {}", file_path.display()))?;
        let mut bytes = vec![0_u8; size];
        file.read_exact(&mut bytes).with_context(|| format!("failed to read piece {}", file_path.display()))?;
        if let Some(expected_hash) = validated.ticket.sha256.as_deref() {
            let actual_hash = hash_bytes(&bytes);
            if !actual_hash.eq_ignore_ascii_case(expected_hash) {
                let _ = write_http_response(&mut stream, "409 Conflict", &[("Content-Length", "0".to_string())], None);
                return Ok(());
            }
        }
        bytes
...
#[tauri::command]
fn set_bandwidth_limit(state: State<'_, AppState>, bandwidth_limit_kbps: i64) -> Result<DesktopStatus, String> {
    let conn = state.connect().map_err(|error| error.to_string())?;
    set_bandwidth_limit_kbps(&conn, bandwidth_limit_kbps).map_err(|error| error.to_string())?;
    let message = if bandwidth_limit_kbps > 0 {
        format!("Bandwidth limit set to {} KB/s", bandwidth_limit_kbps.clamp(0, 1_000_000))
    } else {
        "Bandwidth limit disabled".to_string()
    };
    log_activity(&state, "info", &message)
    .map_err(|error| error.to_string())?;
    current_status(&state).map_err(|error| error.to_string())
}
...
#[tauri::command]
fn reset_local_sync_state(state: State<'_, AppState>) -> Result<DesktopStatus, String> {
    let conn = state.connect().map_err(|error| error.to_string())?;
    conn.execute_batch(
        r#"
        ...
        DELETE FROM raid_status_cache;
        UPDATE raid_local_shards SET status = 'available', updated_at = CURRENT_TIMESTAMP WHERE status <> 'available';
        "#,
    )
    .map_err(|error| error.to_string())?;
    log_activity(&state, "warn", "Local sync state was reset by the user").map_err(|error| error.to_string())?;
    current_status(&state).map_err(|error| error.to_string())
}
...
pub fn run() {
    ...
    if let Err(error) = run_inner() {
        ...
        let message = format!(
            "Torrent Shred Vault Desktop could not start.\n\n{error:#}\n\nLogs: {}",
            log_path.display()
        );
        append_desktop_log("error", &message);
        show_startup_error("Torrent Shred Vault startup failed", &message);
    }
}

fn run_inner() -> Result<()> {
    let state = AppState::new().context("failed to initialize desktop state")?;
    tauri::Builder::default()
        ...
        .map_err(|error| anyhow!("error while running tauri application: {error}"))?;
    Ok(())
}
