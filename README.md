...
Docker-first local vault control plane with Postgres metadata, JWT auth, user/share vaults, desktop peer sync, admin backups, and an admin update workflow.
...
- Backups: the `backup` sidecar stores encrypted Postgres/config backups in a Docker volume; vault blob bytes are excluded by default
...
The admin panel has separate menus for Overview, Users, RAID Reserve, Backups, System Update, and Settings. System Update checks `https://github.com/pranto48/torrent-shred-vault.git` branch `main`, rebuilds Docker images, and recreates containers without deleting Docker volumes.
...
- Large peer downloads use piece manifests with per-piece hashes and can resume verified pieces after restart/interruption.
- Remote desktop sync requires routable LAN/VPN connectivity for this phase.
