...
app.use((req, res, next) => {
  if (req.path === '/api/files/upload' || req.path === '/api/sync/files/upload' || req.path === '/api/chunks/upload') return next();
...
function peerTicketPayload({ userId, sourceDeviceId, targetDeviceId, vaultId, path, rootKind }) {
  return {
    userId,
    sourceDeviceId,
    targetDeviceId,
...
async function publishPeerSource(client, { userId, vault, deviceId, rootKind, relativePath, revision, sizeBytes, sha256 }) {
  await client.query(
    `INSERT INTO sync_peer_sources (user_id, vault_id, device_id, root_kind, path, revision, size_bytes, sha256)
     ...
     DO UPDATE SET
       ...
       updated_at = NOW()`,
    [userId, vault.id, sanitizeDeviceId(deviceId), rootKind, normalizeRelativePath(relativePath), revision, sizeBytes, sha256],
  );
}
...
async function unpublishPeerSource(client, { userId, vault, deviceId, relativePath }) {
  await client.query(
    ...
    [userId, vault.id, sanitizeDeviceId(deviceId), normalizeRelativePath(relativePath)],
  );
}
...
app.get('/api/sync/raid/status', authenticate, authorize('user', 'admin'), async (req, res) => {
  ...
  try {
    ...
    await refreshRaidHealthForUser(client, req.user.sub);
    const manifests = await client.query(
      `SELECT id, vault_id, path, revision, size_bytes, shard_bytes, status, quorum_count, source_device_id, updated_at
       FROM raid_manifests
       WHERE user_id = $1 AND status IN ('pending', 'protected', 'degraded', 'repairing')
    ...
    const results = [];
    for (const row of manifests.rows) {
      const summary = await getRaidManifestSummary(req.user.sub, row.vault_id, row.path, Number(row.revision), sanitizeDeviceId(req.query.deviceId));
      if (!summary) continue;
      results.push(summary);
    }
    const reserve = await client.query(
      `SELECT raid_reserve_bytes
       FROM vault_packages
       WHERE user_id = $1`,
      [req.user.sub],
    );
    const reserveUsage = await getRaidReserveUsage(client, req.user.sub);
    await client.query('COMMIT');
    return res.json({
