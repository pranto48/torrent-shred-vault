...
Phase 3 turns the Docker web/API application into the desktop sync control plane. Android remains a later phase and must use the API contracts documented here instead of direct database access.

## Already Built

- Docker Compose stack with web, API, Postgres, optional legacy MinIO, and updater services.
...
- `desktop/dist` build and `cargo check` verification passing.

Still pending after this phase:

- Tauri updater plugin with signed in-app install/restart flow.
- Android client implementation.
- Full torrent-style swarm transport and RAID repair from hosted shard peers without any full-file peer source.
